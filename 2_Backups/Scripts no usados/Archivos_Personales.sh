...
#Variables Generales
password=""
nombreBackup="Archivos_Personales`date +%y`_MES`date +%b`_DIA`date +%d`"
...
7z a -t7z -r -m0=lzma -mx=9 -mfb=64 -md=32m -ms=on -mhe=on -p$password ~/8_Backups/0-TMP/$nombreBackup.7z -x{!lost+found,!.Trash*} /media/fryntiz/DATOS_2TB/7-Archivos_Personales

mv ~/8_Backups/0-TMP/$nombreBackup.7z ~/8_Backups/Archivos_Personales/$nombreBackup.7z
